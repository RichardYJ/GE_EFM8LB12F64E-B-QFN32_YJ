...
void main(void) {
	...
	while (1) {
		...
		for (data_count = 0; data_count < NUM_BYTES_WR; data_count++) {
			SMB_DATA_OUT[data_count] = 0;    //dat; // Define next outgoing byte
			dat++;
		}
#endif	  
...
#if 0
SMB0_I2C_MasterWrite(uint16_t RegAddr, uint16_t RegValue)
{
...
int GE_I2C2_HexWrite(uint16_t RegAddr, uint16_t RegValue)
{
   ...
   uint8_t dataValue[]= {0x00,0x00,0x00,0x00};

   dataAddr[2]=((RegAddr>>8)&0xff);
   dataAddr[3]=(RegAddr);

...
bool SMB0_I2C_MasterWrite(uint16_t RegAddr, uint16_t RegValue) {
	...
	SMB_DATA_OUT[1] = ((RegAddr >> 8) & 0xff);
	SMB_DATA_OUT[0] = (RegAddr);
...
void main(void) {
	...
	volatile uint8_t i;                          // Dummy variable counters
...
//      TARGET = SLAVE_ADDR;             // Target the Slave for next SMBus
		...
		for(i=0;i<16;i++){
			...
			printf("\r\n SMB_DATA_IN: GE_REG[%d] ->0x%02X ",i,sI2C_rd);
		...
		if(1==SMB0_I2C_MasterWrite(0x9811,0xabcd))
		{
			sI2C_rd = SMB0_I2C_MasterRead(0x00);
			if(0xabcd != sI2C_rd)
				printf("SMB0_I2C Write Error !!!");
			printf("\r\n SMB_DATA_IN: [0][1]->0x%02X ", sI2C_rd);
		}else
			printf("SMB0_I2C Write Failed !!!");
#endif
